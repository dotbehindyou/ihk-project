...
namespace SM.Managers
{
    public class CustomerManager : BaseManager {
        public CustomerManager(BaseManager bm = null)
            : base(bm)
        {
        ...
        public Customer Create(Int32 kdnr)
        {
            ...
            customer.Name = Mapper.ExecuteScalar<String>("SELECT KDNAMI FROM KDSTM where KDKDNR = ? and KDWERK = ? and KDKZDK = 'D'", true,
                new OdbcParameter("kdnr", kdnr),
                new OdbcParameter("werk", Config.Current.Werk));

             Mapper.ExecuteQuery("INSERT INTO SM_Customers (Kdnr, Auth_Token) VALUES (?,?)", true,
                new OdbcParameter("Kdnr", kdnr),
        ...
        {
             Mapper.ExecuteQuery("UPDATE SM_Customers SET Modified = now(), Auth_Token = ? where Kdnr = ?", true,
                new OdbcParameter("auth_token", customer.Auth_Token),
        ...
        {
             Mapper.ExecuteQuery("UPDATE SM_Customers SET Deleted = now() where kdnr = ?", true,
                new OdbcParameter("kdnr", kdnr));
        }
        ...
        public List<Customer> GetMany(Search search = null)
        {
            ...
            parms.Add(new OdbcParameter("werk", Config.Current.Werk));
            ...
            foreach (var cus in  Mapper.GetMany<SM_Customers>("select top 1000 KDKDnr as Kdnr, KDSTM.KDNAMI as Name, SM_Customers.Auth_Token, cast(case when SM_Customers.Kdnr is null then 0 else 1 end as bit) as IsRegisterd from KDSTM " +
                        ...
                        "order by IsRegisterd desc, Kdnr", true, parms.ToArray()))
            {
        ...
        public Customer Get(Int32 kdnr)
        {
            SM_Customers c =  Mapper.GetSingle<SM_Customers>("select SM_Customers.Kdnr, KDSTM.KDNAMI as Name, SM_Customers.Auth_Token from SM_Customers " +
                "left join KDSTM on KDKDNR = SM_Customers.Kdnr and KDWERK = ? and KDKZDK = 'D' where SM_Customers.Kdnr = ?", true,
                new OdbcParameter("werk", Config.Current.Werk),
                new OdbcParameter("Kdnr", kdnr));
        ...
        {
            return  Mapper.ExecuteScalar<Int32>("SELECT Kdnr FROM SM_Customers where Auth_Token = ?", true,
